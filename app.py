...
def analyze_image(image):
    ...
    # Check if image is already a PIL Image object or a file path
    if isinstance(image, Image.Image):
        img = image.convert("RGB")
    else:
        img = Image.open(image).convert("RGB")
    
    # Enhanced prompt for wound classification and reasoning
    prompt = (
        "You are a medical AI assistant analyzing wound images. "
        "Please analyze this wound image and provide: "
        "1. Wound Type Classification (choose from: Abrasion, Burn, Laceration, Puncture, Ulcer) "
        "2. Detailed reasoning for the classification based on visible features "
        "3. Key characteristics observed in the wound "
        "4. Potential severity indicators "
        "Format your response as: "
        "CLASSIFICATION: [wound type] "
        "REASONING: [detailed explanation] "
        "CHARACTERISTICS: [key features observed] "
        "SEVERITY_INDICATORS: [potential severity factors] "
        "Keep the analysis objective and medical-focused."
    )
...
def parse_gemini_response(response_text):
    """Parse the Gemini response to extract classification and reasoning"""
    ...
    try:
        ...
        for line in lines:
            ...
            elif line.startswith("SEVERITY_INDICATORS:"):
                severity = line.replace("SEVERITY_INDICATORS:", "").strip()
                reasoning += f"\n\nSeverity Indicators: {severity}"
        
        # If parsing failed, use the entire response as reasoning
        if classification == "Unknown" and "Unknown" not in response_text:
            # Try to extract classification from the text
            wound_types = ["Abrasion", "Burn", "Laceration", "Puncture", "Ulcer"]
            for wound_type in wound_types:
                if wound_type.lower() in response_text.lower():
...
def classify_wound_image(img):
    ...
    # Show loading state
    loading_prediction = """
    <div class='wound-card' style='padding: 20px; background-color: #1e1e1e; border-radius: 12px;
                box-shadow: 0 0 10px rgba(0,0,0,0.5);'>
        <div style='font-size: 22px; font-weight: bold; color: orange; margin-bottom: 10px;'>
            Predicted Wound Type
        </div>
        <div class='wound-card-content' style='font-size: 16px; color: #cccccc; text-align: center;'>
    ...
    # Use Gemini model for classification and reasoning
    gemini_response = analyze_image(img)
    predicted_class, reasoning_text = parse_gemini_response(gemini_response)

    # Prediction Card with consistent height
    predicted_card = f"""
    <div class='wound-card' style='padding: 20px; background-color: #1e1e1e; border-radius: 12px;
                box-shadow: 0 0 10px rgba(0,0,0,0.5);'>
        <div style='font-size: 22px; font-weight: bold; color: orange; margin-bottom: 10px;'>
        ...
        <div class='wound-card-content' style='font-size: 26px; color: white;'>
            {predicted_class}
        </div>
    </div>
    """

    # Reasoning Card with consistent height
    reasoning_card = f"""
    <div class='wound-card' style='padding: 20px; background-color: #1e1e1e; border-radius: 12px;
                box-shadow: 0 0 10px rgba(0,0,0,0.5);'>
        <div style='font-size: 22px; font-weight: bold; color: orange; margin-bottom: 10px;'>
        ...
        <div class='wound-card-content' style='font-size: 16px; color: white; line-height: 1.6;'>
            {reasoning_text}
        </div>
    </div>
    """

    return predicted_card, reasoning_card, predicted_class
...
def get_gemini_severity_analysis(image, stats, wound_classification="Unknown"):
    """Get AI-powered severity analysis using Gemini model"""
    if image is None:
        return "Unknown", "Unable to analyze image with AI."
    ...
    # Create detailed prompt with wound metrics
    prompt = f"""
    You are a medical AI specialist analyzing wound severity. Please examine this wound image carefully and provide a comprehensive medical assessment based on both visual characteristics and the following quantitative measurements:

    WOUND CLASSIFICATION FROM INITIAL ANALYSIS: {wound_classification}
    
    VISUAL ANALYSIS INSTRUCTIONS:
    - Examine the wound appearance, color, edges, surrounding tissue
    - Look for signs of infection, necrosis, granulation tissue
    - Assess tissue viability and healing indicators
    - Consider any visible exudate or drainage
    
    QUANTITATIVE DEPTH MEASUREMENTS:
    - Mean Depth: {stats['mean_depth_mm']:.1f} mm
    - Maximum Depth: {stats['max_depth_mm']:.1f} mm
    - Depth Standard Deviation: {stats['depth_std_mm']:.1f} mm
    - Deep Tissue Ratio: {stats['deep_ratio']*100:.1f}%
    
    AREA MEASUREMENTS:
    - Total Wound Area: {stats['total_area_cm2']:.1f} cm²
    - Superficial Area: {stats['superficial_area_cm2']:.1f} cm²
    - Partial Thickness Area: {stats['partial_thickness_area_cm2']:.1f} cm²
    - Full Thickness Area: {stats['full_thickness_area_cm2']:.1f} cm²
    - Deep Area: {stats['deep_area_cm2']:.1f} cm²
    ...
    Based on your visual examination of the wound image AND the quantitative measurements above, please provide:
    1. SEVERITY_LEVEL: Choose from (Superficial, Mild, Moderate, Severe, Very Severe)
    2. MEDICAL_ASSESSMENT: Detailed medical analysis combining visual and depth findings (3-4 sentences)
    3. CLINICAL_SIGNIFICANCE: What this means for treatment and prognosis (2-3 sentences)
    4. RECOMMENDATIONS: Specific treatment recommendations based on both appearance and depth (2-3 points)
    5. RISK_FACTORS: Potential complications to monitor based on visual and depth analysis

    Format as:
    SEVERITY_LEVEL: [level]
    MEDICAL_ASSESSMENT: [detailed assessment combining visual and quantitative findings]
    CLINICAL_SIGNIFICANCE: [clinical meaning and prognosis]
    RECOMMENDATIONS: [specific treatment recommendations]
    RISK_FACTORS: [potential complications based on visual and depth analysis]
    """
    
    try:
        response = model.generate_content([prompt, pil_image])
        return parse_gemini_severity_response(response.text)
    except Exception as e:
        return "Moderate", f"AI analysis failed: {str(e)}. Using fallback assessment based on measurements."

def parse_gemini_severity_response(response_text):
    """Parse Gemini response for severity analysis"""
    if not response_text:
        return "Unknown", "No response from AI analysis."
    
    # Default values
    severity_level = "Unknown"
    medical_assessment = "Analysis unavailable"
    ...
    try:
        ...
        for line in lines:
            ...
            if line.startswith("SEVERITY_LEVEL:"):
                severity_level = line.replace("SEVERITY_LEVEL:", "").strip()
            elif line.startswith("MEDICAL_ASSESSMENT:"):
                medical_assessment = line.replace("MEDICAL_ASSESSMENT:", "").strip()
                ...
                risk_factors = line.replace("RISK_FACTORS:", "").strip()
        
        # Combine all information
        full_assessment = f"{medical_assessment}"
        if clinical_significance:
            full_assessment += f" {clinical_significance}"
        if recommendations:
            full_assessment += f" Recommended treatment: {recommendations}"
        if risk_factors:
            full_assessment += f" Monitor for: {risk_factors}"
            
    except Exception as e:
        full_assessment = f"Error parsing AI response: {str(e)}"
    
    return severity_level, full_assessment

def analyze_wound_severity(image, depth_map, wound_mask, pixel_spacing_mm=0.5, depth_calibration_mm=15.0, wound_classification="Unknown"):
    """Enhanced wound severity analysis with AI-powered medical assessment"""
    ...
    ai_severity, ai_assessment = get_gemini_severity_analysis(image, stats, wound_classification)
    ...
    }.get(ai_severity, "#9E9E9E")    # Gray for unknown

    # Create comprehensive medical report with AI analysis
    report = f"""
    <div style='padding: 20px; background-color: #1e1e1e; border-radius: 12px; box-shadow: 0 0 10px rgba(0,0,0,0.5);'>
        ...
        <div style='background-color: #2c2c2c; padding: 15px; border-radius: 8px; margin-bottom: 20px;'>
            <div style='font-size: 18px; font-weight: bold; color: #ffffff; margin-bottom: 10px;'>
                🔬 Wound Classification Reference
            </div>
            <div style='color: #cccccc; line-height: 1.6;'>
                <div>🏷️ <b>Classified Wound Type:</b> {wound_classification}</div>
            </div>
        </div>
...
with gr.Blocks(css=css, title="Wound Analysis & Depth Estimation") as demo:
    ...
    shared_wound_classification = gr.State(value="Unknown")
    ...
    with gr.Tabs():
        ...
        with gr.Tab("1. Wound Classification"):
            ...
            with gr.Row():
                ...
                with gr.Column(scale=1):
                    wound_prediction_box = gr.HTML(value="""
                    <div class='wound-card' style='padding: 20px; background-color: #1e1e1e; border-radius: 12px;
                                box-shadow: 0 0 10px rgba(0,0,0,0.5);'>
                        <div style='font-size: 22px; font-weight: bold; color: orange; margin-bottom: 10px;'>
                            Predicted Wound Type
                    ...
                    """)
                    wound_reasoning_box = gr.HTML(value="""
                    <div class='wound-card' style='padding: 20px; background-color: #1e1e1e; border-radius: 12px;
                                box-shadow: 0 0 10px rgba(0,0,0,0.5);'>
                        <div style='font-size: 22px; font-weight: bold; color: orange; margin-bottom: 10px;'>
                            AI Reasoning
            ...
            wound_image_input.change(fn=classify_wound_image, inputs=wound_image_input,
                                   outputs=[wound_prediction_box, wound_reasoning_box, shared_wound_classification])
        ...
        with gr.Tab("3. 🩹 Wound Severity Analysis"):
            ...
            def run_auto_severity_analysis(image, depth_map, pixel_spacing, depth_calibration, wound_classification):
                if depth_map is None:
                    return """
                    <div style='padding: 30px; background-color: #1e1e1e; border-radius: 12px; box-shadow: 0 0 10px rgba(0,0,0,0.5); text-align: center;'>
                        ...
                        <div style='font-size: 16px; color: #cccccc;'>
                            Please load depth map from Tab 2 first.
                        </div>
                    </div>
                    """
                ...
                # Analyze severity using the automatic mask with wound classification
                return analyze_wound_severity(image, depth_map, processed_mask, pixel_spacing, depth_calibration, wound_classification)
            ...
            auto_severity_button.click(
                ...
                inputs=[severity_input_image, severity_depth_map, pixel_spacing_slider, depth_calibration_slider, shared_wound_classification],
                outputs=[severity_output]
            )
